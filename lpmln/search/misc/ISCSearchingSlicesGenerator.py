...
def generate_isp_slices_by_isc_metadata(kmn, max_task_slice_size=1000, min_non_empty_iset_number=1, max_non_empty_iset_number=6):
    metadata = meta.get_kmn_isc_meta_data(*kmn)
    unknown_iset_number = len(metadata.se_iset_ids)
    if max_non_empty_iset_number < 0:
        max_non_empty_iset_number = unknown_iset_number

    slice_file = config.get_task_slice_file_path_by_kmn(*kmn, min_non_empty_iset_number, max_non_empty_iset_number)

    generate_isp_slices(max_task_slice_size, min_non_empty_iset_number, max_non_empty_iset_number, unknown_iset_number, slice_file)




def generate_isp_slices_by_rule_number(rule_number, is_use_extended_rules, max_task_slice_size=1000, min_non_empty_iset_number=1, max_non_empty_iset_number=6):
    rule_set_size = 3
...
if __name__ == '__main__':
    ...
    generate_isp_slices_by_rule_number(rule_number=rule_number, is_use_extended_rules=is_use_extened_rules, max_non_empty_iset_number=max_ne_iset_number)
